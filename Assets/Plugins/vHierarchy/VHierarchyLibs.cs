    ...
    public static class VUtils
    {
        ...
        public static class EditorUtils
        {
            ...
            public static void OpenFolder(string path)
            {
                ...
                m_ListAreaState.GetType().GetField("m_SelectedInstanceIDs").SetValue(m_ListAreaState, new List<int> { folder.GetObjectId() });
        ...
        // Unity 6.3+ marks InstanceID/EntityId/SceneHandle int conversions obsolete (CS0619 as error).
        // Keep int-based plugin state via a single compatibility boundary.

#if UNITY_6000_3_OR_NEWER
        public static int ToInt(this EntityId id) => unchecked((int)EntityId.ToULong(id));
        public static EntityId ToEntityId(this int id) => EntityId.FromULong(unchecked((ulong)id));
        public static int GetObjectId(this Object o) => o != null ? unchecked((int)EntityId.ToULong(o.GetEntityId())) : 0;
        public static int GetHandleId(this UnityEngine.SceneManagement.Scene scene) => unchecked((int)scene.handle.GetRawData());
        ...
        static void _GlobalObjectId_GlobalObjectIdentifiersToInstanceIDsSlow(GlobalObjectId[] identifiers, int[] outputInstanceIDs)
        {
            ...
            for (int i = 0; i < outputEntityIds.Length; i++)
                outputInstanceIDs[i] = outputEntityIds[i].ToInt();
#else
