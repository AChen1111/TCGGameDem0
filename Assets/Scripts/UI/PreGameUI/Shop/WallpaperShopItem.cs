...
/// <summary>壁纸商品数据.PriceGold 和 Owned 暂无对应 UI 元素,先随数据带上,点击时输出到日志.</summary>
public class WallpaperShopItemData
{
    public int Id { get; }
    ...
    public WallpaperShopItemData(int id, string name, Sprite sprite, long priceGold, bool owned, int index)
    {
        Id = id;
        Name = name;
        Sprite = sprite;
...
/// <summary>商城里的单个壁纸格子.挂在行预制体的子 Button 上.</summary>
public class WallpaperShopItem : MonoBehaviour
{
    ...
    void OnClick()
    {
        ...
        ALog.Log(
            $"商城壁纸点击: Id={m_Data.Id}; Name={m_Data.Name}; PriceGold={m_Data.PriceGold}; Owned={m_Data.Owned}.",
            ALogCategories.UI);
