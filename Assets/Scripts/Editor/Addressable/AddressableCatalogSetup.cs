...
public static class AddressableCatalogSetup
{
    ...
    const string AutoGeneratedStart = "//--tag:auto-generated";
    const string AutoGeneratedEnd = "//--tag:auto-generated-end";
    ...
    public static void SyncAddressKeys()
    {
        ...
        string existing = File.ReadAllText(AddressKeysPath);
        string text = ReplaceAutoGenerated(existing, generated);
        if (existing.Replace("\r\n", "\n") == text.Replace("\r\n", "\n"))
        {
            return;
        }
    ...
    // 只替换起止标记之间的内容,其后手写方法不受影响
    static string ReplaceAutoGenerated(string existing, string generated)
    {
        int start = existing.IndexOf(AutoGeneratedStart);
        int end = existing.IndexOf(AutoGeneratedEnd);
        if (start < 0 || end <= start)
        {
            return BuildFullAddressKeys(generated);
        }

        int startLineEnd = existing.IndexOf('\n', start);
        if (startLineEnd < 0)
        {
            return BuildFullAddressKeys(generated);
        }

        return existing.Substring(0, startLineEnd + 1) + generated + existing.Substring(end);
    }

    static string BuildFullAddressKeys(string generated)
    {
        var sb = new StringBuilder();
        sb.AppendLine("public static class AddressKeys");
        sb.AppendLine("{");
        sb.AppendLine("    " + AutoGeneratedStart);
        sb.Append(generated);
        sb.AppendLine("    " + AutoGeneratedEnd);
