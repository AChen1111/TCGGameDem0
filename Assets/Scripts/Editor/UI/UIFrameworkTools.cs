...
/// <summary>创建 UIFrame 层级骨架(Panel/Window/Para 层)的编辑器工具.</summary>
public static class UIFrameworkTools
{
    // 与 Assets/Editor/Common/EditorMenus 的 Tools 分组约定一致.
    const string Menu = "Tools/UI/";

    [MenuItem(Menu + "创建 UI Frame（场景）", priority = 2)]
    public static void CreateUIFrameInScene() {
        CreateUIFrame(EventSystem.current == null);
    }

    [MenuItem(Menu + "创建 UI Frame Prefab", priority = 1)]
    public static void CreateUIFramePrefab() {
        var frame = CreateUIFrame(true);
    ...
    private static GameObject CreateUIFrame(bool includeEventSystem) {
