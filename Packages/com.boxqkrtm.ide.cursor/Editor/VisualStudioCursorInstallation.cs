...
";
		...
		{
			// Cursor 3 可能把 Unity 的打开请求路由到 Agents Window, 强制使用经典编辑器窗口.
			arguments = $"--classic {arguments}";
...
#if UNITY_EDITOR_WIN
		private static void RestoreWindowsPath(ProcessStartInfo startInfo)
		{
			try
			{
				// Unity 可能传入损坏的 PATH, 从注册的系统与用户环境重新构建后再启动 Cursor.
				var machinePath = Environment.GetEnvironmentVariable("PATH", EnvironmentVariableTarget.Machine);
				var userPath = Environment.GetEnvironmentVariable("PATH", EnvironmentVariableTarget.User);
				var registeredPaths = new[] { machinePath, userPath }
					.Where(value => !string.IsNullOrWhiteSpace(value));
				var restoredPath = string.Join(";", registeredPaths);
				if (!string.IsNullOrWhiteSpace(restoredPath))
					startInfo.EnvironmentVariables["PATH"] = restoredPath;
			}
			catch (Exception ex)
			{
				Debug.LogWarning($"[Cursor] Failed to restore PATH before launch: {ex.Message}");
			}
		}
#endif
